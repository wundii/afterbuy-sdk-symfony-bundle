...
use Wundii\AfterbuySdk\SymfonyBundle\Tests\XsdSchemaValidationTest;

class YamlSchemaValidationTest extends TestCase
{
    public function testServicesYamlIsValidYaml(): void
    {
        $path = __DIR__ . '/../src/Resources/config/packages/afterbuy_sdk.yaml';
        ...
        $namespace = 'http://wundii.com/schema/dic/afterbuy_sdk';
        $dom = new DOMDocument('1.0', 'UTF-8');
        $dom->formatOutput = true;

        $config = $dom->createElementNS($namespace, 'config');
        $dom->appendChild($config);
        $this->arrayToXml($parsed['afterbuy_sdk'], $config, $dom, $namespace);
